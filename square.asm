...
square_loop2:
    ...
    OR(R0, R7, R0)    |; R0 <- (R0 || R7)

    |; Second binary comparaison
    CMPEQ(R6, R1, R7) |; R7 <- (xCur (R6) == xTopLeft (R1))
    CMPEQ(R6, R4, R8) |; R8 <- (xCur (R6) == xBottomRight (R4))
    OR(R7, R8, R7)    |; R7 <- (R7 || R8)

    OR(R0, R7, R0)    |; Combine both conditions: is it a border pixel?
